...
// Find the start of updateHeroUI and the start of renderTimeline to replace everything between them
...
const correctUpdateHeroUIAndRenderUI = `function updateHeroUI(isCurrentlyOn, nextChangeTime, isAllClear) {
        ...
        // Show Tomorrow Button if available (dev restriction: after 20:00)
        const btnTomorrow = document.getElementById('btn-show-tomorrow');
        if (btnTomorrow && !window.isTomorrowView) {
            const tomorrow = new Date();
            tomorrow.setDate(tomorrow.getDate() + 1);
            
            const d = tomorrow.getDate().toString().padStart(2, '0');
            const m = (tomorrow.getMonth() + 1).toString().padStart(2, '0');
            const tomorrowStr = \`\${d}.\${m}\`;

            const currentHour = new Date().getHours();
            const showAfterHour = 20;

            if (scheduleData && scheduleData[tomorrowStr] && currentHour >= showAfterHour) {
                btnTomorrow.classList.remove('hidden');
            } else {
...
const newCode = code.slice(0, startIndex) + correctUpdateHeroUIAndRenderUI + code.slice(endIndex);
